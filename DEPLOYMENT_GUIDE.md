# RDAS Kubernetes Deployment & Troubleshooting Guide

This guide describes how to deploy, verify, and troubleshoot the **Reference Data Aggregation Service (RDAS)** on a Kubernetes cluster.
...
RDAS includes a multi-stage [Dockerfile](file:///Users/paul/Documents/vibing/reference-data-aggregator/Dockerfile) that packages Java 21 / Spring Boot into a minimal alpine JRE image.

```bash
docker build -t loopdfs/rdas:1.2.0 .
```
...
The ConfigMap [k8s/configmap.yaml](file:///Users/paul/Documents/vibing/reference-data-aggregator/k8s/configmap.yaml) contains the environmental configuration for SOAP endpoints, scheduler crons, caching sizes, and persistence paths:
...
The main workload manifests are consolidated in [k8s/statefulset.yaml](file:///Users/paul/Documents/vibing/reference-data-aggregator/k8s/statefulset.yaml):
...
This will deploy:
* **StatefulSet (rdas):** A 3-replica set where each pod gets its own Persistent Volume Claim (PVC) to preserve the local cache snapshot JSON, protecting against SOAP outages on container rescheduling.
* **Service (rdas):** A headless/ClusterIP service exposing port `80` to route internal traffic to container port `8080`.
* **HorizontalPodAutoscaler (rdas-hpa):** Autoscales replicas from 3 to 10 based on CPU (70%) and Memory (80%) thresholds.
* **PodDisruptionBudget (rdas-pdb):** Restricts voluntary disruptions to ensure at least 2 replicas remain available during upgrades or maintenance.
...
```bash
kubectl rollout status statefulset/rdas
```

### 2. Validate Pod Status & PV Claims
Ensure that 3 replicas are running and their PVC volume mounts are bound:
```bash
kubectl get pods -l app=rdas
kubectl get pvc -l app=rdas
```

### 3. Verify Health Probes (Actuator)
Check the actuator endpoints directly. You can port-forward to a pod (e.g. `rdas-0`):
```bash
kubectl port-forward pod/rdas-0 8080:8080
```
...
* **Verify Cache Startup State:**
  ...
  * Confirm that the pod can resolve and access the external SOAP endpoint:
    ```bash
    kubectl exec -it rdas-0 -- curl -I "http://webservices.oorsprong.org/websamples.countryinfo/CountryInfoService.wso?WSDL"
    ```
  * If DNS fails, inspect coreDNS/networking inside the cluster.

### 4. Data Staleness & Fallback Audits
When the SOAP upstream goes down, the application will fallback to local disk storage (`/data/snapshot/rdas_snapshot.json`) or classpath baseline.
...
* **Reset Circuit Breaker (or Force Cache Refresh):**
  If the upstream is recovered and you want to bypass the hourly retry scheduling or wait times, you can trigger a manual cache refresh by triggering:
  * *(Admin refresh details can be added or triggered via configuration restart to flush current circuit breaker states)*:
  ```bash
  kubectl rollout restart statefulset/rdas
  ```
